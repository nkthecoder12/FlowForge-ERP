...
import { inventoryReserveSchema } from '@/lib/server/modules/inventory/inventory.validation';
...
export async function POST(request: Request) {
  return handleRoute(async () => {
    ...
    const parsed = inventoryReserveSchema.safeParse(body);
    ...
    return jsonSuccess(result, 'Stock reserved successfully', 201);
  });
}
