...
import { rejectManufacturingOrderSchema } from '@/lib/server/modules/manufacturing/manufacturing.validation';
...
export async function POST(request: Request, context: RouteContext) {
  return handleRoute(async () => {
    ...
    const parsed = rejectManufacturingOrderSchema.safeParse(body);
    ...
    const result = await manufacturingService.reject(
      id,
      parsed.data.reason,
      actor.sub,
      actor.name,
    ...
    return jsonSuccess(result, 'Manufacturing order rejected successfully');
  });
}
