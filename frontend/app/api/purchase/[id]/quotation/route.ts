...
import { selectQuotationSchema } from '@/lib/server/modules/purchase/purchase.validation';
...
export async function POST(request: Request, context: RouteContext) {
  return handleRoute(async () => {
    ...
    const parsed = selectQuotationSchema.safeParse(body);
    ...
    const result = await purchaseService.selectQuotation(
      id,
      parsed.data,
      actor.sub,
      actor.name,
    ...
    return jsonSuccess(result, 'Vendor quotation selected successfully');
  });
}
