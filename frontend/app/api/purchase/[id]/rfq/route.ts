...
import { sendRFQSchema } from '@/lib/server/modules/purchase/purchase.validation';
...
export async function POST(request: Request, context: RouteContext) {
  return handleRoute(async () => {
    ...
    const parsed = sendRFQSchema.safeParse(body);
    ...
    const result = await purchaseService.sendRFQ(
      id,
      parsed.data,
      actor.sub,
      actor.name,
    ...
    return jsonSuccess(result, 'Procurement verified and RFQs sent successfully');
  });
}
